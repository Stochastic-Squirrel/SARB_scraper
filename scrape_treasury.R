...
extract_indicator_items <- function(){
  ...
  #create a column to indicate the position of each timeseries indicator for each indicator
  indicator_codes %>% group_by(Code) %>% mutate(timeseries_position = 1 ) %>% mutate(timeseries_position = cumsum(timeseries_position) )
...
scrape_online_statistical_query <- function(custom_selection=NULL , cores = 3){
  ...
  if(is.null(custom_selection)){
    ...
    indicator_codes <- fread("indicator_codes.csv") %>% filter(Frequencies !='Daily (6 Days)') %>%
      filter(ts_code %in% custom_selection)
    
  }
  
  #changed from passing indicator to passing explicit timeseries code
  #code_list <- unique(indicator_codes$code_number)
  code_list <- indicator_codes$ts_code
  ...
  parallel_results <-foreach (index = 1:length(code_list),.errorhandling = "pass",.verbose = TRUE )  %dopar%
  {
    ...
    #From the timeseries code, extract the indicator code which will need to be put into the form
    timeseries_code <- code_list[index]
    code <- filter(indicator_codes, ts_code == timeseries_code)$code_number
   ...
   # for (timeseries in 1:time_series_number){
    {
      #timesries represents position of checkbox for desired timeseries code
      timeseries <- filter(indicator_codes, ts_code == timeseries_code)$timeseries_position
